...
class SharedPreference (application: Context) {
    private val PREFS_NAME = "CAB_SIGNATURE"
    ...
    var customerId: String
        get() = encryptDataHelper.decryptData(
            sharedPref.getString(CUSTOMER_ID, "").toString(), AppCash.token
        )
        set(customerId) = sharedPref.edit().putString(
            CUSTOMER_ID, encryptDataHelper.encryptData(customerId, AppCash.token)
        ).apply()

    var password: String
        get() = encryptDataHelper.decryptData(
            sharedPref.getString(PASSWORD, "").toString(), AppCash.token
        )
        set(password) = sharedPref.edit().putString(
            PASSWORD, encryptDataHelper.encryptData(password, AppCash.token)
        ).apply()
